...
# -*- coding: utf-8 -*-
"""
Created on Tue May  1 19:43:07 2018
...
class human:
    def __init__(self,ball_x,ball_y,velocity_x,velocity_y,paddle_y,player_y, reward):
        self.ball_x = ball_x
        self.ball_y = ball_y
        self.velocity_x = velocity_x
        self.velocity_y = velocity_y
    ...
    def update(self,action):
        self.paddle_y += action_list[action]
        self.paddle_y = max(0, min(1 -paddle_height, self.paddle_y))
        self.player_y = player_y = (pygame.mouse.get_pos())[1]
        self.player_y = max(0, min(500 -paddle_height, self.player_y))
        self.ball_x += self.velocity_x
        self.ball_y += self.velocity_y
        #top
        if self.ball_y < 0:
        ...
        if self.ball_y > 1:
            ...
            self.velocity_y = -self.velocity_y
            #left
        if self.ball_x < 0 and (self.ball_y*500 < self.player_y or self.ball_y*500> self.player_y+0.2*500):
...
def pong(agent):
    ...
    pygame.display.set_caption('CS440-MP4-human vs AI')
   
    total = 0
    for i in range(epoch):
        bounce = 0 
        state = human(0.5, 0.5, 0.03, 0.01, 0.5 - paddle_height / 2, 250, 0) 
        curr_state = state.convert()
        while True:
